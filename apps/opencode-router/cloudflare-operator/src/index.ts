...
import { config, sessionHostname, sessionPortHostname, sessionAttachHostname, sessionEditorHostname } from "./config.js"
...
async function onPodAdded(pod: k8s.V1Pod): Promise<void> {
  ...
  try {
    ...
    // Create editor route if configured
    const editorHostname = sessionEditorHostname(hash)
    if (editorHostname) {
      promises.push(
        createDnsRecord(editorHostname, tunnelCname),
        createTunnelRoute(editorHostname),
        createIngressRoutes(editorHostname),
      )
    }

    await Promise.all(promises)
    console.log(`Provisioned ${hostname} (main port + attach route${editorHostname ? ` + editor route` : ""})`)
...
async function onPodDeleted(pod: k8s.V1Pod): Promise<void> {
  ...
  try {
    ...
    // Remove editor route if configured
    const editorHostname = sessionEditorHostname(hash)
    if (editorHostname) {
      deletePromises.push(deleteTunnelRoute(editorHostname), deleteIngressRoutes(editorHostname))
    }

    await Promise.all(deletePromises)
    console.log(`Removed routing for ${hostname}${editorHostname ? ` + editor` : ""}`)
...
console.log(`  Editor prefix     : ${config.editorRoutePrefix ?? "(disabled)"}`)
